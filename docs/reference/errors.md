...
| `O1009` | expression nesting exceeds depth … | Break the expression into `<let>` steps; deep nesting is capped so checking stays linear. | `src/parser.ts:147` |
| `O1010` | unexpected … after expression | Delete the trailing token, or wrap the whole thing in parentheses if you meant one expression. | `src/parser.ts:102` |
| `O1011` | unexpected end of expression | The expression ends mid-operator — supply the missing operand. | `src/parser.ts:114 (+1 more)` |
| `O1012` | expected … | Insert the punctuation the parser named (usually a `)`, `]`, `}` or `:`). | `src/parser.ts:128` |
| `O1013` | the right side of \|> must be a filter name | The right of `\|>` is a filter name, optionally with arguments: `x \|> truncate(40)`. | `src/parser.ts:241` |
| `O1014` | expected a property name after | Write a static property name after the dot. There is no dynamic member access. | `src/parser.ts:312` |
| `O1015` | dynamic member access is not supported | Use a static property (`obj.name`). `obj["name"]` would make data flow unanalyzable. | `src/parser.ts:329` |
| `O1016` | method calls are not supported | Methods do not exist; pipe instead: `value \|> filter(args)`. | `src/parser.ts:347` |
...
| `O5001` | stored AST has no templates record | The stored AST has no `templates` record — the row is not an Orbit program. | `src/validate-ast.ts:446` |
