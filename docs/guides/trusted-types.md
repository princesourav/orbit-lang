...
## The one place they meet: `unsafeHtml` filters
...
2. **Audit the use sites.** Every one is a compile-time warning, so a build log
   is a complete inventory. Grep for `unsafeHtml: true` and you have the list of
   filters; the warnings give you the list of templates.

If you can avoid an `unsafeHtml` filter entirely — by giving authors structured
