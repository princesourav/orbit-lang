...
The 2025–26 CVE record makes this the right decade to say it loudly: LiquidJS RCE at CVSS 10.0, four Handlebars CVEs in one release, six-plus Twig sandbox bypasses in one cycle, Jinjava at 9.8 — all deny-list retrofits over Turing-complete evaluators with host-object reachability ([Orca on LiquidJS](https://orca.security/resources/blog/critical-rce-in-liquidjs-lets-attackers-execute-arbitrary-commands-on-unpatched-hosts/), [Symfony on CVE-2026-46634](https://symfony.com/blog/cve-2026-46634-template-from-string-escapes-a-sourcepolicy-driven-sandbox-via-synthesized-template-name), [GHSA-gjx9-j8f8-7j74](https://github.com/advisories/GHSA-gjx9-j8f8-7j74)).

**Beachheads, ranked by who can embed us today.** Orbit ships as an npm TypeScript library, so the first two beachheads are JS-native: **(1) AI template-generation platforms** (typed compile errors give agents a repair loop; no incumbent occupies this), **(2) edge/Workers-for-Platforms operators** ([market wedge](https://www.cloudflare.com/products/workers-for-platforms/)) — with the explicit caveat that edge performance claims are gated on a reproducible benchmark (see v1.0). **(3) Multi-tenant SaaS embedding customer templates** (Braze/Zendesk/HubSpot-class) is the largest prize but runs Ruby/Java/Python/PHP backends; it is sequenced third and unlocked by an explicit portability story: a documented non-JS embedding guide (sidecar render service, precompiled signed-AST pipeline) at v1.0, and a Rust/WASM implementation post-1.0. We do not compete for Shopify's storefront or for trusted-developer codegen speed, and we deliberately offer **no Liquid→Orbit migration tooling** — adoption is greenfield-first: new template estates (AI-generated themes, new platform builds), not conversions of incumbent ones.

## Strategic pillars

**P1 — "No sandbox because no escape surface."** Non-Turing-completeness, closed allowlists (95 elements, banned `script`/`style`/`iframe`/`svg`), six structurally-assigned escaping contexts, and branded terminal types (`Html`, `Money`, `Url`) are the product. The codebase already delivers this with unusual discipline (RAWTEXT unreachable by construction; `Html` interpolation in TEXT is the *only* unescaped sink). Every 2026 sandbox CVE class maps to an Orbit construct that makes it unrepresentable — that mapping becomes a flagship document, published early ([security research](https://thehackernews.com/2026/07/n8n-sandbox-escape-lets-workflow.html)).
...
**P3 — Spec + first-party conformance suite as the moat.** Deterministic byte-exact rendering makes a CommonMark-style executable spec uniquely feasible ([CommonMark model](https://spec.commonmark.org/0.30/)). Liquid's suite (golden-liquid) had to come from a third party; Orbit ships it first-party, which is also what makes safety claims falsifiable and AI agents trainable ([golden-liquid](https://github.com/jg-rp/golden-liquid/blob/main/README.md), [Willison on conformance suites](https://simonwillison.net/2025/Dec/31/the-year-in-llms/)). Correctness is validated externally — differential testing against browser parsers, and later a second implementation — never solely against the implementation that generated the suite.

**P4 — Platform-operator features nobody else ships.** Fuel/deadline/output budgets and static data-access-plan extraction are green-field: no public engine offers either, Shopify enforces equivalents privately, and Twig's own CVE-2026-46627 admits "sandbox does not protect against resource exhaustion" ([competitors](https://github.com/shopify/liquid); [Cloudflare limits](https://developers.cloudflare.com/workers/platform/limits/)). These sell to the platform operator, not the template author. The access-plan analysis is load-bearing for later features (LSP completions, fragment-cache keys), so its known soundness gaps are fixed in v0.2, before anything depends on it.

**P5 — AI agents as a first-class audience — delivered early, not last.** Shopify's July '26 Liquid preview is explicitly motivated by agent legibility ([Shopify changelog](https://shopify.dev/changelog/developer-preview-liquid-block-and-partial-tags)); no shipping product markets a typed, non-Turing template language as an LLM codegen target ([DSL partitioning](https://bradmurry.com/software/dsl-ai-partition/)). Typed compile errors give agents a machine-checkable repair loop; budgets bound autonomous blast radius. Because this is the strongest 2026 tailwind and is docs-plus-harness cheap, the prompt pack and eval harness ship in v0.5, not post-1.0. This audience also machine-audits us — Anthropic's Project Glasswing found 10,000+ high/critical OSS issues in a month ([Help Net Security](https://www.helpnetsecurity.com/2026/05/26/anthropic-project-glasswing-update/)) — so minimal surface + written spec is survival, not marketing.

**P6 — Match the Liquid tooling bar, greenfield-first.** Shopify ships a VS Code extension with Theme Check, completions, and formatting ([Shopify Liquid VS Code](https://shopify.dev/docs/storefronts/themes/tools/shopify-liquid-vscode)); "better safety with worse tooling than Liquid loses." Orbit deliberately ships no migration tooling from Liquid or any incumbent: conversions would import untyped idioms and set false drop-in-compatibility expectations. The adoption path is new templates — AI-generated, new platform builds, new tenant estates — so the tooling investment goes entirely into making authoring-from-scratch (by humans and agents) faster than porting.
...
Goal: better tooling than Liquid for our niche, following the proven small-team order ([Gleam playbook](https://gleam.run/news/v0.21-introducing-the-gleam-language-server/)), and an end to public silence.
...
- **LLM prompt kit prototype (Q4 2026)**: system-prompt pack teaching Orbit's grammar, types, and error codes, plus an **eval harness** (generate → compile → repair-loop success rate) built on the existing test corpus. Docs-plus-harness cheap, and it is the wedge no incumbent occupies. llms.txt + llms-full.txt ship with the docs site ([Astro's removal was mis-measurement](https://dacharycarey.com/2026/05/04/astro-removed-llms-txt/)).
...
- **First edition of the "CVE class → unrepresentable construct" page** (moved up from v1.0; the normative version follows with the spec) — published while LiquidJS/Twig/Jinjava are still fresh news.
...
- Open **paid design-partner conversations pre-1.0** with at least three publicly wounded platforms (LiquidJS embedders, Jinjava/HubL-class, n8n-class); target one signed paid pilot or LOI before launch. The closed layer (hosted rendering/governance) is validated from day one, not after.
...
- **"CVE class → unrepresentable construct" security page, normative edition**: LiquidJS `valueOf`, Twig `template_from_string`, Jinjava introspection, Handlebars codegen concat, n8n sanitizer — each mapped to the Orbit design that excludes it.
...
- **Bench harness, one number only**: a reproducible harness directory with a single headline measurement — Workers CPU-ms for a representative product-listing render on the *current* engine. This single benchmark is exempt from the cut-performance-first rule, because the edge pitch is unsupported without it; broader benchmarks wait for the VM. **No speed claims in the README** — a correctness-comparison table competitors can't dispute instead ([benchmarketing risk](https://bitsondata.dev/what-is-benchmarketing-and-why-is-it-bad)).
...
- **LLM codegen kit, expanded**: the v0.5 prompt pack and eval harness grow into an MCP docs server and design-partner program aimed at the publicly wounded (LiquidJS embedders, Jinjava/HubL-class platforms, n8n-class workflow tools).
...
**Metrics:** ≥2 production multi-tenant embeddings; ≥1 AI-builder partner generating Orbit in production; second implementation at ≥95% conformance [community]; closed layer generating revenue from ≥1 paying design partner converted from the pre-1.0 LOIs — the engine alone captures zero revenue ([Liquid open-core lesson](https://digiday.com/marketing/shopify-partner-ecosystem-800-million/)).
...
| 4 | **Strictness rejection**: real users hit missing power (dynamic style, i18n, email HTML) and bounce to Liquid | Sanctioned escape valves (host filters now; locale-data injection at v1.0; profiles and micro-contexts via staged proposals); honest limitation docs from v0.2 so nothing fails as a surprise; never answer with Turing-completeness |
